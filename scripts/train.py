...
from config import MODEL_NAME, COMBINED_FILE, MAX_LENGTH, BATCH_SIZE, EPOCHS, LEARNING_RATE, SOURCE_LANG, TARGET_LANG, DEVICE

CHECKPOINT_DIR = "models/nllb_finetuned/checkpoints"
...
def main(args):
    ...
    # Load dataset and optionally limit
    raw_dataset = load_dataset("csv", data_files={"train": COMBINED_FILE}, split="train")
    if args.limit:
    ...
    # Start training (resumes if checkpoint exists)
    trainer.train(resume_from_checkpoint=last_checkpoint)
    trainer.save_model(args.output_dir)

    # Save GenerationConfig
    gen_config = GenerationConfig.from_model_config(model.config)
    gen_config.save_pretrained(args.output_dir)
    print(f"Training complete. Model saved to {args.output_dir}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune NLLB-200 with checkpoint support")
    parser.add_argument("--output_dir", type=str, default="model_finetuned", help="Directory for saving model/checkpoints")
